...
d3.csv('./xun.csv', function(dataIn){
    ...
    scaleX.domain(loadData.map(function(d){return d.stock;}));
    scaleY.domain([0, d3.max(loadData.map(function(d){return +d.price}))]);
...
function drawPoints(pointData){
    ...
    scaleY.domain([0, d3.max(pointData.map(function(d){return +d.price}))]);
    ...
    svg.selectAll('rect')
        ...
        .attr('height',function(d){
            return 400 - scaleY(d.price);  //400 is the beginning domain value of the y axis, set above
        });
